...
class AProjectController extends Controller
{
    ...
    public function rename(Request $request)
    {
        $validator = $this->getValidationFactory()
            ->make($request->all(), [
                'url' => 'required|string|min:3|max:20',
                'name' => 'required|string|min:3|max:20',
            ]);
        if ($validator->fails()) {
            return response(['error' => $validator->errors(), 'Validaton Error'], 400);
        ...
        $this->saveHistory("Rename project " . $request->url);

        $curl_task = curl_init();
        curl_setopt($curl_task, CURLOPT_URL, $user->nextcloudurl . $request->url);
        curl_setopt($curl_task, CURLOPT_CUSTOMREQUEST, "PROPPATCH");
        curl_setopt($curl_task, CURLOPT_POSTFIELDS, '<x0:propertyupdate xmlns:x0="DAV:"><x0:set><x0:prop><x0:displayname>' . $request->name . '</x0:displayname><x6:calendar-color xmlns:x6="http://apple.com/ns/ical/">#0082c9</x6:calendar-color></x0:prop></x0:set></x0:propertyupdate>');
        curl_setopt($curl_task, CURLOPT_RETURNTRANSFER, true);
